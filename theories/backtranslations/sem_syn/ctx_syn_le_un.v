...
From st.backtranslations.sem_syn Require Import nr_embed_project nr_guard_assert nr_no_op connective.nr_syn_le_un gamma_lib uni_back_ctx.
...
Section syn_le_un_S_n.
  ...
  Lemma subst_list_val_anti_steps vs (Hl : length vs = length Γ) :
  ...
  Proof.
    ...
    clear pn pCscp pe. revert Hl. revert Γ.
  ...
  Lemma wrap_funs_vals_help (Γ' : list nr_type) :
    ∀ (F F' : expr) vs vs' (Hl : length vs = length Γ'),
      ⊢ exprel_typed s (GammaType (fmap nr_type_type Γ') τr) F F' -∗ ([∗ list] v;v' ∈ vs;vs', valrel v v') -∗
        exprel_typed s τr
  ...
  Proof.
    ...
    - iIntros (F F' vs vs' Hl) "HF Hlist".
      ...
      iApply nr_project_connective; auto. iIntros (w w') "Hww".
  ...
  Lemma rel_uni_back_ctx_cons_e_le_ctx_e :
    ⊢ exprel_typed s TUnit (fill_ctx (uni_back_ctx_cons C Γ τr) e) (fill_ctx C e)  .
  Proof.
    ...
    (* step *)
    simpl. iApply lift_step_later. rewrite -> pn at 1. rewrite fill_LamGamma_ctx. rewrite -LamGammaV_S_rw. auto_lam_step. iEval simplify_custom. change (Lam ?e) with (of_val (LamV e)). rewrite LamGammaV_S_rw -pn. rewrite ep_pair_Closed. iNext.
    (* connective lemma *)
    iApply (nr_project_connective_expr NRTUnit).
    ...
    (* introducing stuff *)
    iIntros (vs vs' Hl) "#Hvs". simpl. rewrite ep_pair_Closed.
    (* collecting info *)
    iDestruct (big_sepL2_length with "Hvs") as "%Hl'".
    (* connective lemma *)
    iApply (nr_embed_connective_expr τr).
    (* rewriting substitutions *)
    ...
    { eapply expr_Closed_n, (expr_typed_scoped [] _ _).
      apply Ht. } rewrite Hc.
    rewrite !wrap_funs_vars_subst2'. assert (Hl'' : length vs' = length Γ) by by rewrite -Hl' Hl.
    iApply (lift_anti_steps_spec _ _ _ _ _ (subst_list_val_anti_steps vs' Hl'')).
    iApply wrap_funs_vals_help; eauto.
    iDestruct (auto_related_typed_expr _ _ _ _ Ht [] []) as "H". asimpl.
    by iApply "H".
    (* remaining goals *)
    all: auto; try rewrite fmap_length; try lia.
    all: by apply Forall_fmap, Forall_true; intros τ σ; rewrite ep_pair_Closed.
  Qed.
...
Section syn_le_un_0.
  ...
  Lemma rel_uni_back_ctx_nil_e_le_ctx_e :
    ⊢ exprel_typed s TUnit (fill_ctx (uni_back_ctx_nil C τr) e) (fill_ctx C e)  .
  Proof.
    simpl.
    iApply (nr_project_connective_expr NRTUnit).
