...
From st.lam Require Import lang ghost_steps types typing tactics logrel.definitions logrel.generic.lift reducibility wkpre fixlam.
From st Require Import resources.
From st.backtranslations.sem_syn Require Import guard_assert.

Section GhostStep_ga.
  ...
  Definition TP (τ : type) (grd asr : val) : iProp Σ :=
    (∀ (v v' : val), (valrel_typed s τ v v' -∗ exprel_typed s τ (GhostStep v) (grd v'))) ∗
    (∀ (v v' : val), (valrel_typed s τ v v' -∗ exprel_typed s τ (GhostStep v) (asr v'))).

  Lemma GhostStep_ga_help :
    ∀ (τ : type) (τs : list type) (pCnτ : Closed_n (length τs) τ) (gas : list val),
        □ ([∗ list] τ ; ga ∈ τs ; gas , TP τ (LamV (Fst (ga.{ren (+1)} ()) %0)%Eₙₒ) (LamV (Snd (ga.{ren (+1)} ()) %0))%Eₙₒ) ⊢
          ∀ ga v v', valrel_typed s τ.[subst_list τs] v v' -∗ exprel_typed s τ.[subst_list τs] (GhostStep v) ((ga_pair ga τ).{subst_list_val gas} v').
  Proof.
    iLöb as "IHlob".
    ...
    - iClear "IHlob". iIntros (τs pτ gas) "#Hgas".
      ...
      + iApply lift_step_later. auto_lam_step. simplify_custom. iNext.
        iApply lift_step. auto_lam_step. simplify_custom.
        iApply lift_step. auto_lam_step. simplify_custom.
        change ()%Eₙₒ with (of_val ()%Vₙₒ). iApply lift_val.
        by rewrite valrel_typed_TUnit_unfold /=.
    - iClear "IHlob". iIntros (τs pτ gas) "#Hgas".
      ...
      + iApply lift_step_later. auto_lam_step. simplify_custom. iNext.
        iApply lift_step. auto_lam_step. simplify_custom.
        iApply lift_step. destruct b; auto_lam_step. simplify_custom.
        change (Lit b)%Eₙₒ with (of_val (b)%Vₙₒ). iApply lift_val.
        rewrite valrel_typed_TBool_unfold /=. by iExists _.
    - iClear "IHlob". iIntros (τs pτ gas) "#Hgas".
      iIntros (ga v v') "#Hvv'".
      ...
      + iApply lift_step_later. auto_lam_step. simplify_custom. iNext.
        iApply lift_step. auto_lam_step. simplify_custom.
        iApply lift_step. auto_lam_step. simplify_custom. replace (z + 0%nat)%Z with z by lia.
        change (Lit z)%Eₙₒ with (of_val (z)%Vₙₒ). iApply lift_val.
        rewrite valrel_typed_TInt_unfold /=. by iExists _.
    - setoid_rewrite <- val_subst_valid. iClear "IHlob". iIntros (τs pτ gas) "#Hgas".
      ...
      iApply lift_step. auto_lam_step. iEval (rewrite -!val_subst_valid; simplify_custom).
    ...
    - setoid_rewrite <- val_subst_valid. iClear "IHlob". iIntros (τs pτ gas) "#Hgas".
      ...
      iApply lift_step. auto_lam_step. iEval (simplify_custom). iEval (rewrite -!val_subst_valid; simplify_custom).
      ...
      iApply (lift_bind'' _ [AppRCtx _; GhostStepCtx] [AppRCtx _; AppRCtx _]).
      iEval (rewrite -!val_subst_valid). iApply "IH". iNext.
      iIntros (r r') "#Hrr'". iEval simpl. iSpecialize ("Hvv'" $! r r' with "Hrr'").
      iApply (lift_bind'' _ [GhostStepCtx] [AppRCtx _] with "Hvv'"). iIntros (s s') "#Hss'/=".
    ...
    - setoid_rewrite <- val_subst_valid. iIntros (τs pμτ gas) "#Hgas".
      ...
      iApply lift_step_later. auto_lam_step. iNext. iEval simplify_custom.
      iApply lift_step. auto_lam_step. iEval simplify_custom.
  ...
  Lemma GhostStep_ga (τ : type) (pCτ : Closed τ) ga (v v' : val) :
    valrel_typed s τ v v' ⊢ exprel_typed s τ (GhostStep v) (ga_pair ga τ v').
  Proof.
    cut (valrel_typed s τ.[subst_list []] v v' ⊢ exprel_typed s τ.[subst_list []] (GhostStep v) ((ga_pair ga τ.[subst_list []]).{subst_list_val [] } v')).
    rewrite pCτ. asimpl. by rewrite val_subst_ids.
    iIntros "#Hvv'".
    iDestruct (GhostStep_ga_help τ [] pCτ []) as "H/=". asimpl. by iApply "H".
  Qed.

  Lemma GhostStep_ga_expr (τ : type) (pCτ : Closed τ) ga (e e' : expr) :
    exprel_typed s τ e e' ⊢ exprel_typed s τ (GhostStep e) (ga_pair ga τ e').
  Proof.
    iIntros "Hee'".
    iApply (lift_bind'' _ [GhostStepCtx] [AppRCtx _] with "Hee'").
    iIntros (v v') "#Hvv'". by iApply GhostStep_ga.
  Qed.
