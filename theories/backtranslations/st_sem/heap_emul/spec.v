...
From st.lam Require Import lang wkpre tactics lib.fixlam.
...
Lemma read_i_at_least_one (vs : list val) (v : val) (i : nat) (H : vs !! i = Some v) :
  at_least_one lam_step (read_i i (encode vs)) (encode vs, v)%Vₙₒ.
Proof.
  simpl.
...
Lemma write_i_v_step i v (vs : val) :
  lam_step (write_i_v i v vs) (write_i_v_vs i v vs).
Proof.
...
Lemma write_i_v_vs_steps (vs : list val) (i : nat) (v v' : val) (piv : vs !! i = Some v) :
  rtc lam_step (write_i_v_vs i v' (encode vs)) (encode (<[i := v']> vs), ())%Vₙₒ.
Proof.
...
Lemma write_i_v_at_least_one (vs : list val) (i : nat) (v v' : val) (piv : vs !! i = Some v) :
  at_least_one lam_step (write_i_v i v' (encode vs)) (encode (<[i := v']> vs), ())%Vₙₒ.
Proof.
  eapply at_least_one_l. auto_lam_step. simplify_custom.
  apply (rtc_lam_step_ctx (fill [PairLCtx _])).
  rewrite insert_length.
  eapply rtc_transitive.
  apply (rtc_lam_step_ctx (fill [PairLCtx _])).
...
Lemma write_i_step (i : val) (v : val) : lam_step (write_i i v) (write_i_v i v).
Proof.
  rewrite /write_i.
  apply head_prim_step. eapply App_Lam_head_step'.
...
Lemma write_step (i : val) : lam_step (write i) (write_i i).
Proof.
  rewrite /write.
  apply head_prim_step. eapply App_Lam_head_step'.
...
(*   Proof. *)
(*     iIntros "Hauth Hi Hu". iApply wp_step_later. auto_lam_step. simplify_custom. iNext. *)
...
Lemma bind_v_f_step (v r1 r2 s1 f h0 h1 h2 : lam.lang.val) :
  rtc lam_step (v h0) (h1, r1)%Eₙₒ →
  rtc lam_step (f r1) s1 →
  rtc lam_step (s1 h1) (h2, r2)%Eₙₒ →
  rtc lam_step (bind_v_f v f h0) (h2, r2)%Eₙₒ.
Proof.
  intros H1 H2 H3.
  rewrite /bind_v_f.
  rtc_step.
  eapply rtc_transitive. by apply (rtc_lam_step_ctx (fill_item (LetInCtx _))). simpl.
  do 5 rtc_step.
  eapply rtc_transitive. by apply (rtc_lam_step_ctx (fill_item (AppLCtx _))). auto.
Qed.
