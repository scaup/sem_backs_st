From st.STLCmu Require Import types lang.
...
Reserved Notation "Γ ⊢ₙₒ e : τ" (at level 74, e, τ at next level).

Inductive typed (Γ : list type) : expr → type → Prop :=
 | Var_typed x τ : Γ !! x = Some τ → Γ ⊢ₙₒ Var x : τ
...
Lemma typed_subst_invariant Γ e τ s1 s2 :
  Γ ⊢ₙₒ e : τ → (∀ x, x < length Γ → s1 x = s2 x) → e.[s1] = e.[s2].
Proof.
...
Lemma n_closed_invariant_type n (e : type) s1 s2 :
  (∀ f, e.[upn n f] = e) → (∀ x, x < n → s1 x = s2 x) → e.[s1] = e.[s2].
Proof.
  intros Hnc. specialize (Hnc (ren (+1))).
  revert n Hnc s1 s2.
...
Lemma typed_n_closed Γ τ e : Γ ⊢ₙₒ e : τ → (∀ f, e.[upn (length Γ) f] = e).
Proof.
...
Lemma context_strengthening ξ Γ e τ :
  ξ ++ Γ ⊢ₙₒ e.[(ren (+ (length ξ)))] : τ → Γ ⊢ₙₒ e : τ.
...
Lemma context_swap_typed Γ' ξ' ξ Γ e τ :
  Γ' ++ ξ' ++ ξ ++ Γ ⊢ₙₒ e : τ →
  Γ' ++ ξ ++ ξ' ++ Γ ⊢ₙₒ e.[upn (length Γ') (ren (lookup_n (swap_list (length ξ) (length ξ'))))] : τ.
Proof.
  intros Ht.
...
Lemma context_gen_rename Γ e τ n m :
  Γ ⊢ₙₒ e : τ → subst (upn n (ren (+m))) <$> Γ ⊢ₙₒ
                     e : τ.[upn n (ren (+m))].
Proof.
...
Lemma context_rename Γ e τ m :
  Γ ⊢ₙₒ e : τ → subst (ren (+m)) <$> Γ ⊢ₙₒ e : τ.[ren (+m)].
Proof.
...
Lemma typed_gen_subst Γ1 Γ2 e1 τ1 e2 τ2 :
  Γ1 ++ τ2 :: Γ2 ⊢ₙₒ e1 : τ1 →
  Γ2 ⊢ₙₒ e2 : τ2 →
  Γ1 ++ Γ2 ⊢ₙₒ e1.[upn (length Γ1) (e2 .: ids)] : τ1.
Proof.
...
Lemma typed_subst Γ2 e1 τ1 e2 τ2 :
  τ2 :: Γ2 ⊢ₙₒ e1 : τ1 → Γ2 ⊢ₙₒ e2 : τ2 → Γ2 ⊢ₙₒ e1.[e2/] : τ1.
