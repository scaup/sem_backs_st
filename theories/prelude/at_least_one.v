...
Proof. intros. destruct (rtc_nsteps _ _ H0) as [n Hn]. exists (S n). split; [lia | ]. eapply nsteps_l; eauto. Qed.

Lemma at_least_one_r {A : Type} (R : relation A) (x y z : A) : rtc R x y → R y z → at_least_one R x z.
Proof. intros. destruct (rtc_nsteps _ _ H) as [n Hn]. exists (S n). split; [lia | ]. eapply nsteps_r; eauto. Qed.

Definition at_least_one_rtc {A : Type} (R : relation A) x y : at_least_one R x y → rtc R x y.
Proof. destruct 1 as [n [_ H]]. by eapply nsteps_rtc. Qed.
...
Proof. destruct 1 as [n [H H']]. destruct H'. lia. exists y. split; auto. by eapply nsteps_rtc. Qed.

Lemma at_least_one_split_r {A : Type} (R : relation A) x z : at_least_one R x z → ∃ y, rtc R x y ∧ R y z.
Proof. destruct 1 as [n [H H']]. destruct n. lia. destruct (nsteps_inv_r _ _ _ H') as [y [P P']]. exists y. split; auto. by eapply nsteps_rtc. Qed.
