...
From st.backtranslations.sem_syn.sem_le_syn Require Import guard_assert ga_ctx.
From st.cheap_extra Require Import connective.
...
Section ep_ctx_le_ga_ctx_0.
  ...
  Lemma rel_ep_ctx_le_ga_ctx_0 :
    ctx_rel_typed MaybeStuck (back_ctx_nil C τ) (ga_ctx_nil C τ) [] τ [] TUnit.
  Proof.
    iIntros (e e' pe pe' Hee').
    apply open_exprel_typed_nil. simpl.
    iApply project_assert_connective_expr. by intro σ; asimpl.
    rewrite -!fill_ctx_app.
    iApply open_exprel_nil'. simpl.
    apply (universe_back_ctx_in_relation 0 0 C pCscp).
    apply open_exprel_nil.
    iApply embed_guard_connective_expr. auto.
    by iApply open_exprel_typed_nil'.
  Qed.

End ep_ctx_le_ga_ctx_0.

Section ep_ctx_le_ga_ctx_S_n.
  ...
  Proof.
    ...
    - iIntros (HCΓ' F F' vs vs' Hl) "HF Hlist".
      ...
      iApply project_assert_connective; auto. by inversion HC2. iIntros (w w') "Hww".
  ...
  Lemma rel_ep_ctx_le_ga_ctx_S_n :
    ctx_rel_typed MaybeStuck (back_ctx_cons C Γ τ) (ga_ctx_cons C Γ τ) Γ τ [] TUnit.
  Proof.
    ...
    (* connective lemma *)
    iApply project_assert_connective_expr; first by intro σ; asimpl. iNext.
...
End ep_ctx_le_ga_ctx_S_n.

Section ep_ctx_le_ga_ctx.
  ...
  Lemma rel_ep_ctx_le_ga_ctx :
    ctx_rel_typed MaybeStuck (back_ctx C Γ τ) (ga_ctx C Γ τ) Γ τ [] TUnit.
  Proof.
    destruct (length Γ) eqn:eq.
    - destruct Γ; [|by inversion eq]. by apply rel_ep_ctx_le_ga_ctx_0.
    - rewrite /back_ctx /ga_ctx. rewrite eq. eapply rel_ep_ctx_le_ga_ctx_S_n; eauto.
      by rewrite eq.
  Qed.

End ep_ctx_le_ga_ctx.
