...
From st.lam Require Import lang typing types types contexts scopedness logrel.definitions logrel.adequacy logrel.fundamental.
...
Section back_ctx_sem_syn.
  ...
  Lemma back_ctx_sem_syn_correct_emulation :
    ∀ (e : expr) (de : Γ ⊢ₙₒ e : τ),
      lam_halts (fill_ctx (back_ctx_sem_syn) e) <-> lam_halts (fill_ctx C e).
  Proof.
    intros e de. split.
    - (* C_b[e] ⇓ ⇒ C[e] *)
      intro SynHalts.
    ...
    - (* C[e] ⇓ ⇒ C_b[e] *)
      intro SemHalts.
      (* C_s[e] ⇓, where C_s is C wrapped with our GhostStep's *)
      assert (lam_halts (fill_ctx (gs_ctx C (length Γ)) e)).
      { (* using logical relatedness *)
      ...
      assert (lam_halts (fill_ctx (ga_ctx C Γ τ) e)).
      { (* using logical relatedness *)
